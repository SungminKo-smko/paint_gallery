...
function restoreFromBuffer(displayWidth, displayHeight) {
  ...
  ctx.drawImage(drawingBuffer, 0, 0, drawingBuffer.width, drawingBuffer.height, 0, 0, displayWidth, displayHeight);
}
...
function openViewModal(item) {
  ...
  viewImage.src = item.imageData;
  viewImage.alt = item.title;
  viewMeta.textContent = `${item.title} · ${item.visibility === 'public' ? '공개' : '비공개'}`;
...
function renderSavedDrawings() {
  ...
  saved.forEach((item) => {
    ...
    if (item.visibility === 'public') publicGrid.prepend(card);
    else privateGrid.prepend(card);
  });
}
...
function deleteSelectedDrawing() {
  ...
  const saved = getSavedDrawings();
  const next = saved.filter((x) => x.id !== selectedDrawingId);
...
colorPicker.addEventListener('input', () => { if (!eraserMode) ctx.strokeStyle = colorPicker.value; });
brushSize.addEventListener('input', () => {
  ctx.lineWidth = Number(brushSize.value);
  brushSizeLabel.textContent = brushSize.value;
});
eraserBtn.addEventListener('click', () => { eraserMode = true; ctx.strokeStyle = '#ffffff'; showToast('지우개 모드'); });
penBtn.addEventListener('click', () => { eraserMode = false; ctx.strokeStyle = colorPicker.value; showToast('펜 모드'); });
...
deleteDrawingBtn.addEventListener('click', deleteSelectedDrawing);
